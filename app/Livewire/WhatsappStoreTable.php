...
class WhatsappStoreTable extends LivewireTableComponent
{
  ...
  public function builder(): Builder
  {
    if (auth()->user()->hasRole('super_admin')) {
      return WhatsappStore::query()->with('tenant.user')->join('users', 'users.tenant_id', '=', 'whatsapp_stores.tenant_id')->select('whatsapp_stores.*');
    } else {
  ...
  public function columns(): array
  {
    return [
      ...
      Column::make(__('messages.vcard.user_name'), 'tenant.tenant_username')
        ...
        ->view('sadmin.whatsapp_stores.columns.user_name')
        ->sortable(function (Builder $query, $direction) {
          return $query->orderBy('users.first_name', $direction)->orderBy('users.last_name', $direction);
        })
