    ...
    try {
      ...
      if (isset($input['product_icon']) && ! empty($input['product_icon'])) {
        ...
        foreach ($files as $file) {
          // Auto-resize product icon to 100x100
          $image = Image::make($file);
          $image->fit(100, 100);

          $tempPath = sys_get_temp_dir() . '/' . uniqid() . '_product_icon.png';
          $image->save($tempPath, 100, 'png');

          $resizedProductIcon = new \Illuminate\Http\UploadedFile(
            $tempPath,
            'product_icon.png',
            'image/png',
            null,
          ...
          $vcardProduct->newAddMedia($resizedProductIcon)->toMediaCollection(
