    ...
    try {
      ...
      if ($vcard->cover_type == 0) {
        if (isset($input['cover_img']) && !empty($input['cover_img'])) {
          ...
          $vcard->cover_url = $input['selected_cover_image'];
          $vcard->save();
        }
    ...
    try {
      ...
      if ($vcard->cover_type == 0) {
        if (isset($input['cover_img']) && !empty($input['cover_img'])) {
          ...
          $vcard->addMedia($input['cover_img'])->withManipulations([
            '*' => ['w' => 360, 'h' => 1170, 'fit' => 'crop'],
          ])->toMediaCollection(Vcard::COVER_PATH, config('app.media_disc'));
          $vcard->youtube_link = "";
          $vcard->cover_url = null; // Clear predefined cover URL when uploading custom
          $vcard->save();
        } elseif (isset($input['selected_cover_image']) && !empty($input['selected_cover_image'])) {
          // Handle predefined cover image - store the URL directly
          $vcard->clearMediaCollection(Vcard::COVER_PATH); // Clear custom uploads
          $vcard->cover_url = $input['selected_cover_image'];
          $vcard->youtube_link = "";
          $vcard->save();
        }
