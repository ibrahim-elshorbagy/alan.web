  ...
  public function store($input)
  {
    try {
      ...
      if (($input['type'] == Gallery::TYPE_IMAGE) && isset($input['image']) && ! empty($input['image'])) {
        // Auto-resize gallery image to 576x300
        $image = Image::make($input['image']);
        $image->fit(576, 300);

        $tempPath = sys_get_temp_dir() . '/' . uniqid() . '_gallery_image.png';
        $image->save($tempPath, 100, 'png');

        $resizedGalleryImage = new \Illuminate\Http\UploadedFile(
          $tempPath,
          'gallery_image.png',
          'image/png',
          null,
