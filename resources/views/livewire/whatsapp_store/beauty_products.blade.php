      ...
      <div class="mb-30">
        ...
        <div>
          <div class="form-check mb-2">
            <input
              class="form-check-input {{ app()->getLocale() == 'ar' || app()->getLocale() == 'fa' ? 'float-end' : '' }}"
              type="radio" name="flexRadioDefault" id="flexRadioDefault1" wire:model.live="dateFilter" value="3_days" />
            <label
              class="form-check-label fs-16 fw-5 {{ app()->getLocale() == 'ar' || app()->getLocale() == 'fa' ? 'me-4' : '' }}"
          ...
          <div class="form-check mb-2">
            <input
              class="form-check-input {{ app()->getLocale() == 'ar' || app()->getLocale() == 'fa' ? 'float-end' : '' }}"
              type="radio" wire:model.live="dateFilter" value="1_week" name="flexRadioDefault" id="flexRadioDefault2" />
            <label
              class="form-check-label fs-16 fw-5 {{ app()->getLocale() == 'ar' || app()->getLocale() == 'fa' ? 'me-4' : '' }}"
          ...
          <div class="form-check mb-2">
            <input
              class="form-check-input {{ app()->getLocale() == 'ar' || app()->getLocale() == 'fa' ? 'float-end' : '' }}"
              type="radio" wire:model.live="dateFilter" value="1_month" name="flexRadioDefault"
          ...
          <div class="form-check mb-2">
            <input
              class="form-check-input {{ app()->getLocale() == 'ar' || app()->getLocale() == 'fa' ? 'float-end' : '' }}"
              type="radio" wire:model.live="dateFilter" value="6_months" name="flexRadioDefault"
          ...
          <div class="form-check mb-2">
            <input
              class="form-check-input {{ app()->getLocale() == 'ar' || app()->getLocale() == 'fa' ? 'float-end' : '' }}"
              type="radio" wire:model.live="dateFilter" value="1_year" name="flexRadioDefault" id="flexRadioDefault5" />
            <label
              class="form-check-label fs-16 fw-5 {{ app()->getLocale() == 'ar' || app()->getLocale() == 'fa' ? 'me-4' : '' }}"
        ...
        <div>
          @foreach ($categories as $category)
            <div class="category-item">
              <button
                class="category-button w-100 d-flex align-items-center {{ $categorySelected == $category->id ? 'active-category' : '' }}"
                type="button" wire:click="setCategory({{ $category->id }})">
                <div
                  class="category-category-img {{ app()->getLocale() == 'ar' || app()->getLocale() == 'fa' ? 'ms-3' : 'me-3' }}">
                  <img src="{{ $category->image_url }}" class="w-100 rounded" alt="category" />
                </div>
                <span
                  class="{{ app()->getLocale() == 'ar' || app()->getLocale() == 'fa' ? 'text-end flex-grow-1' : 'text-start flex-grow-1' }}">
                  {{ $category->name }}</span>
              </button>
            </div>
          @endforeach
  ...
  <div class="col-xl-9 col-lg-8">
    <div class="row justify-content-end">
      <div class="col-xl-4 col-sm-6 mb-20">
        <div class="position-relative">
          <input type="text" wire:model.live="search"
            placeholder="{{ __('messages.whatsapp_stores_templates.search_products') }}" class="form-control ps-45" />
      ...
      <div class="col-xl-4 col-sm-6 mb-20" wire:ignore>
        <div class="dropdown">
          <button class="serach-dropdown text-start w-100" type="button" id="customSelectBtn" data-bs-toggle="dropdown"
            aria-expanded="false">
        ...
        <div class="col-xl-3 col-sm-6 mb-20">
          <div class="item-card h-100">
            <div class="d-flex flex-column h-100">
              <a href="{{ route('whatsapp.store.product.details', [$urlAlias, $product->id]) }}" style="color: #212529;"
                class="d-block h-100">
                  ...
                  <div class="item-details">
                    ...
                    <p class="fs-18 fw-7">
                      ...
                      @if ($product->net_price)
                        <del
                          class="fs-14 fw-7 text-gray-200">{{ currencyFormat($product->net_price, 2, $product->currency->currency_code) }}</del>
                      @endif
                    </p>
                  </div>
                </div>
