  ...
  <template x-if="hasSelected">
    <div class="d-flex flex-wrap gap-2">
      <button type="button" class="btn btn-warning" @click="syncAndCall('exportSelected')">
        <i class="fas fa-file-export"></i> <span class="d-none d-sm-inline">{{ __('messages.common.export_selected') }}</span>
      </button>

      <button type="button" class="btn btn-success" @click="syncAndCall('markSelectedAsReceived')">
        <i class="fas fa-check"></i> <span class="d-none d-sm-inline">{{ __('messages.redirect_links.mark_selected_as_received') }}</span>
      </button>

      @if (auth()->user()->hasRole('super_admin'))
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#assignModal">
          <i class="fas fa-user-plus"></i> <span class="d-none d-sm-inline">{{ __('messages.redirect_links.assign_selected') }}</span>
        </button>

        <button type="button" class="btn btn-danger"
          @click="if(confirm('{{ __('messages.common.delete_confirm') }}')) { syncAndCall('deleteSelected') }">
          <i class="fas fa-trash"></i> <span class="d-none d-sm-inline">{{ __('messages.common.delete_selected') }}</span>
        </button>
      @endif
      ...
      @if (auth()->user()->hasRole('super_admin'))
        <button type="button" class="btn btn-info"
          ...
          <i class="fas fa-undo"></i> <span class="d-none d-sm-inline">{{ __('messages.redirect_links.restore_selected') }}</span>
        </button>
      @endif
    </div>
  </template>
        ...
        <div class="group-accordion-header {{ $isExpanded ? 'expanded' : '' }}"
          ...
          <div class="group-stats">
            <span class="stat-item">
              <i class="fas fa-list"></i> {{ $allItems->count() }} <span class="d-none d-sm-inline">{{ __('messages.common.items') }}</span>
            </span>
            @if (!auth()->user()->hasRole('sales'))
              <span class="stat-item">
