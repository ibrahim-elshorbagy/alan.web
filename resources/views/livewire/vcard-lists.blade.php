        ...
        <div>
          @if (count($vcards) > 0 || !empty($search))
            <div class="position-relative d-flex align-items-center min-width-220">
              ...
              <input wire:model.debounce.100ms.live="search" type="search" autocomplete="off" class="form-control ps-8"
                id="search" placeholder="{{ __('auth.app.search') }}" aria-label="Search">
            </div>
          @endif
        </div>
        <div class="d-flex justify-content-end ms-auto">
          <div class="btn-group d-flex align-items-end me-2" role="group" aria-label="Basic mixed styles example"
            wire:ignore>
            <button type="button"
                  ...
                  <div class="card h-100 d-block vcard-container position-relative ">
                    <div class="card-body h-100 position-relative p-0 d-flex flex-column justify-content-between">
                      <div class="vcard-edit-icon">
                        <a href="{{ route('vcards.edit', $vcard->id) }}" title="{{ __('messages.common.edit') }}"
                          ...
                          <ul class="dropdown-menu px-2" aria-labelledby="dropdownMenuButton1">
                            ...
                            <li>
                              <a title="{{ __('messages.vcard.download_vcard') }}"
                                ...
                                <i
                                  class="fas fa-download text-info fs-4 mx-2"></i>&nbsp;{{ __('messages.vcard.download_vcard') }}
                              </a>
                            </li>
                            <li>
                              @if (route('enquiry.index', $vcard->id))
                                <a title="{{ __('messages.enquiry') }}"
                                  ...
                                  <i
                                    class="fa-solid fa-clipboard-question text-info fs-4 mx-2"></i>&nbsp;{{ __('messages.contact_us.inquries') }}
                                </a>
                              @endif
                            </li>
                            @if (checkTotalVcard())
                              <li>
                                <a href="javascript:void(0)"
                                  ...
                                  data-id="{{ $vcard->id }}" title="{{ __('Duplicate vCard') }}">
                                  <i
                                    class="fa-solid fa-copy text-secondary fs-4 mx-2"></i>&nbsp;{{ str_replace('!', '', __('messages.vcard.duplicate_vcard')) }}
                                </a>
                                </a>
                              </li>
                            @endif
                            <li>
                              <a href="javascript:void(0)" data-id="{{ $vcard->id }}"
                                title="{{ __('messages.common.delete') }}"
                                class="btn p-1 fs-6 d-flex align-items-center text-danger  vcard_delete-btn">
                                <i class="fa-solid fa-trash fs-4 mx-2"></i>&nbsp;{{ __('messages.common.delete') }}
                              </a>
                            </li>
                            <li>
                              <div class="status">
                                <div class="status">
                                  <button title="{{ __('messages.status') }}"
                                    ...
                                    data-id="{{ $vcard->id }}" value="{{ $vcard->status == 1 ? '0' : '1' }}">
                                    @if ($vcard->status == 1)
                                      <i class="fa fa-toggle-on text-info fs-2 mx-2"></i>
                                      {{ __('messages.vcard.disabled') }}
                                    @else
                                      <i class="fa fa-toggle-off text-info fs-2 mx-2"></i>
                      ...
                      <div class="d-flex flex-column justify-content-between h-100 gap-3 body-content flex-grow-1">
                        <div class="desc p-4">
                          <div class="justify-content-between align-items-center  d-flex">
                            <div>
                              <a href="{{ route('vcards.edit', $vcard->id) }}" class="text-decoration-none fs-6">
                                {{ $vcard->name }}
                              </a>
                            </div>
                            <div>
                              <span class="fs-6 text-gray-500">{{ getFormattedDateTime($vcard->created_at) }}</span>
                            </div>
                          </div>
                          <div class="justify-content-between align-items-center  d-flex">
                            <div>
                        ...
                        <div class="vcard-actions px-4 pb-4">
                          <div class="d-flex w-100 justify-content-center align-items-center">
                            <span class="d-none" id="vcardUrlCopy{{ $vcard->id }}">{{ $vcardUrl }}</span>
                            @if ($vcard->status == 1)
                              <div class="vcard-button show-vcard me-sm-2 me-1">
                                <a href="{{ $vcardUrl }}" id="vcardUrl{{ $vcard->id }}" target="_blank"
                              ...
                              <div class="vcard-button vcard-link-copy mx-sm-2 mx-1">
                                <a class="user-edit-btn vcard-copy-clipboard fs-6" href="javascript:void(0)"
                                  data-id="{{ $vcard->id }}" title="{{ 'copy' }}">
                              ...
                              <div class="vcard-button vcard-Contact mx-sm-3 mx-1">
                                <a href="{{ route('vcard.showContact', $vcard->id) }}">
                            ...
                            @if (analyticsFeature())
                              <div class="vcard-button show-chart mx-sm-3 mx-1">
                                <a href="{{ route('vcard.analytics', $vcard->id) }}" class="fs-6">
