  ...
  <div class="row" id="profile-cover">
    ...
    <div class="col-lg-12 mb-7 cover-imgs">
      <div class="mb-3">
        ...
        <div id="predefinedImages" class="mb-4" style="{{ isset($vcard) && !empty($vcard->cover_url) && str_contains($vcard->cover_url, 'cover_images') ? '' : 'display: none;' }}">
          <div class="card shadow-sm p-4">
            <h5 class="mb-4">{{ __('messages.cover_image.select_cover_image') }}</h5>
            <div class="row g-4" style="max-height: 380px; overflow-y: auto; padding-right: 4px;">
              @php
              ...
              <div class="col-lg-4 col-md-6 col-12">
                <div class="cover-image-option {{ isset($vcard) && $vcard->cover_url == $image->image_url ? 'selected' : '' }}" data-url="{{ $image->image_url }}">
                  <img src="{{ $image->image_url }}" alt="Cover Image" class="img-fluid rounded" style="cursor: pointer; border: 3px solid transparent; transition: all 0.3s;">
                </div>
              </div>
              @empty
          ...
          <input type="hidden" name="selected_cover_image" id="selectedCoverImage" value="{{ isset($vcard) && !empty($vcard->cover_url) && str_contains($vcard->cover_url, 'cover_images') ? $vcard->cover_url : '' }}">
        </div>
        ...
        <div id="customUpload" io-image-input="true" style="{{ isset($vcard) && !empty($vcard->cover_url) && !str_contains($vcard->cover_url, 'cover_images') ? '' : 'display: none;' }}">
          <div class="d-block">
            <div class="images-picker" style="width: 100%; max-width: 600px;">
              <div class="image previewImage" id="coverPreview" style="background-image: url('{{ !empty($vcard->cover_url) && in_array(pathinfo($vcard->cover_url, PATHINFO_EXTENSION), ['jpg', 'jpeg', 'png', 'gif', 'webp']) ? $vcard->cover_url : asset('assets/images/default_cover_image.jpg') }}'); width: 100%; height: 200px; background-size: contain; background-position: center;">
              </div>
...
<div id="floatingSaveBar" class="floating-save-bar d-none">
  <div class="d-flex align-items-center gap-3">
    <span class="text-white small"><i class="fas fa-image me-1"></i>{{ __('messages.cover_image.select_cover_image') }}</span>
    <button type="button" class="btn btn-light btn-sm fw-bold" onclick="$(&#39;#vcardSaveBtn&#39;).trigger(&#39;click&#39;)">{{ __('messages.common.save') }}</button>
  </div>
</div>
...
<script>
  $(document).ready(function() {
    ...
    // Show/hide floating save bar based on predefined panel visibility
    function syncFloatingSaveBar() {
      if ($('#predefinedImages').is(':visible')) {
        $('#floatingSaveBar').removeClass('d-none');
      } else {
        $('#floatingSaveBar').addClass('d-none');
      }
    }
    syncFloatingSaveBar();

    $('input[name="cover_image_source"]').on('change', function() {
      syncFloatingSaveBar();
    });
  });
