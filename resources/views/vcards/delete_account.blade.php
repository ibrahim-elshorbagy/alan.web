        ...
        <div class="card-body d-flex flex-column position-relative align-items-center">
          <div class="ps-sm-3 pt-lg-auto pt-0 w-100 h-100 overflow-auto text-center" id="main">
            <div class="text-center">
              ...
              <h3 class="warning-message"><strong class="text-warning">{{ __('messages.vcard.warning') }}:
                </strong>{{ __('messages.common.user_delete_warning') }}</h3>
              @if ($hasActiveItems)
                <div class="alert alert-danger mt-4">
                  <strong>{{ __('messages.vcard.warning') }}:</strong>
                  {{ __('messages.cannot_delete_user_with_active_vcard_or_redirect_link') }}
                </div>
              @endif
              <button type="button" class="btn btn-danger mt-5" data-bs-toggle="modal" data-bs-target="#userDeleteModal"
                @if ($hasActiveItems) disabled @endif>
                    ...
                    <div class="p-5 text-center">
                      <form method="POST" action="{{ route('delete-user', ['user' => auth()->user()->id]) }}">
                        ...
                        <button type="submit" class="btn btn-danger"
                          data-turbo="false">{{ __('messages.common.account_delete') }}</button>
