...
<ul class="nav nav-tabs nav-items-all-tabs-vcard flex-nowrap gap-3 gap-lg-0 overflow-auto mb-1">
  <li class="text-nowrap nav-item nav-item-tabs">
    <a class="nav-link position-relative p-0 {{ isset($partName) && $partName == 'basics' ? 'active' : '' }}" href="{{ route('vcards.edit', $vcard->id) . '?part=basics' }}" aria-selected="true">{{ __('messages.basic_details1') }}</a>
  </li>
  <li class="text-nowrap nav-item nav-item-tabs">
    <a class="nav-link position-relative p-0 {{ isset($partName) && $partName == 'basics2' ? 'active' : '' }}" href="{{ route('vcards.edit', $vcard->id) . '?part=basics2' }}" aria-selected="false">{{ __('messages.basic_details2') }}</a>
  </li>
  <li class="text-nowrap nav-item nav-item-tabs">
    <a class="nav-link position-relative p-0 {{ isset($partName) && $partName == 'basics3' ? 'active' : '' }}" href="{{ route('vcards.edit', $vcard->id) . '?part=basics3' }}" aria-selected="false">{{ __('messages.basic_details3') }}</a>
  </li>
</ul>
@endif
@endif
...
<div class="container-fluid mt-5">
  <div class="row" id="basic">
    ...
    <div class="col-lg-12 mb-7">
      ...
      <span data-bs-toggle="tooltip" data-placement="top" data-bs-original-title="{{ __('messages.tooltip.the_main_url') }}">
      ...
      <small class="text-muted">{!! __('messages.vcard.url_alias_hint') !!}</small>
      <div id="url-alias-extract-msg" class="text-success ms-1 fs-6 d-none fw-light mt-1">
        <i class="fa-solid fa-circle-check me-1"></i>
        <span id="url-alias-extract-text"></span>
      </div>
      <div id="url-alias-invalid-msg" class="text-danger ms-1 fs-6 d-none fw-light mt-1">
        <i class="fa-solid fa-circle-xmark me-1"></i>
        {{ __('messages.vcard.url_alias_invalid_chars') }}
      </div>
    ...
    {{-- Name field is auto-generated from url_alias --}}
    {{ Form::hidden('name', isset($vcard) ? $vcard->name : null, ['id' => 'vcard-name-hidden']) }}
    ...
    <div class="col-lg-6 mb-7">
      ...
      <div class="mb-5">
        <a href="javascript:void(0)" data-bs-target="#aiDescriptionModal" data-bs-toggle="modal" class="text-primary text-decoration-none fw-semibold d-inline-flex align-items-center gap-2">
  ...
  <div class="col-lg-6 col-sm-6 mb-7 cover-imgs ms-5">
    <div class="mb-3">
      ...
      <span data-bs-toggle="tooltip" data-placement="top" data-bs-original-title="{{ __('messages.tooltip.vcard_cover_img') }}">
        <i class="fas fa-question-circle ml-1 general-question-mark"></i>
      </span>

      <div class="mb-3">
        <div class="form-check form-check-inline">
          <input class="form-check-input" type="radio" name="cover_image_source" id="predefinedCover" value="predefined" {{ !isset($vcard) || (!empty($vcard->cover_url) && !str_contains($vcard->cover_url, 'cover_images')) ? 'checked' : '' }}>
          <label class="form-check-label" for="predefinedCover">
            {{ __('messages.cover_image.predefined_images') }}
          </label>
        </div>
        <div class="form-check form-check-inline">
          <input class="form-check-input" type="radio" name="cover_image_source" id="customCover" value="custom" {{ isset($vcard) && !empty($vcard->cover_url) && str_contains($vcard->cover_url, 'cover_images') ? 'checked' : '' }}>
      ...
      <div id="predefinedImages" class="mb-3" style="{{ !isset($vcard) || (!empty($vcard->cover_url) && !str_contains($vcard->cover_url, 'cover_images')) ? '' : 'display: none;' }}">
        <div class="row">
          @php
          ...
          <div class="col-4 mb-2">
            <div class="cover-image-option {{ isset($vcard) && $vcard->cover_url == $image->image_url ? 'selected' : '' }}" data-url="{{ $image->image_url }}">
        ...
        <input type="hidden" name="selected_cover_image" id="selectedCoverImage" value="{{ isset($vcard) && !empty($vcard->cover_url) && !str_contains($vcard->cover_url, 'cover_images') ? '' : (isset($vcard) ? $vcard->cover_url : '') }}">
      </div>

      <div id="customUpload" io-image-input="true" style="{{ isset($vcard) && !empty($vcard->cover_url) && !str_contains($vcard->cover_url, 'cover_images') ? '' : 'display: none;' }}">
        <div class="d-block">
          <div class="images-picker">
            <div class="image previewImage" id="coverPreview" style="background-image: url('{{ !empty($vcard->cover_url) && in_array(pathinfo($vcard->cover_url, PATHINFO_EXTENSION), ['jpg', 'jpeg', 'png', 'gif', 'webp']) ? $vcard->cover_url : asset('assets/images/default_cover_image.jpg') }}');">
            </div>
            <span class="picker-edit rounded-circle text-gray-500 fs-small" data-bs-toggle="tooltip" data-placement="top" data-bs-original-title="{{ __('messages.tooltip.cover') }}">
              <label>
                <i class="fa-solid fa-pen click-image" id="profileImageIcon"></i>
                <input type="file" id="coverImg" name="cover_img" class="d-none" accept="image/*" data-preview-id="coverPreview" />
              </label>
            </span>
          </div>
          <div data-image-paste data-file-input-id="coverImg" data-preview-id="coverPreview" data-button-text="{{ __('messages.select_image') }}" data-clipboard-button-text="{{ __('messages.paste_from_clipboard') }}" data-success-text="{{ __('messages.image_pasted_successfully') }}" data-invalid-type-text="{{ __('messages.invalid_image_type') }}" data-image-too-large-text="{{ __('messages.image_too_large') }}" data-no-image-text="{{ __('messages.no_image_in_clipboard') }}">
          </div>
        </div>
      </div>
    </div>
...
<div class="col-lg-6 col-sm-4 mb-7">
  <div class="mb-3 ms-5" io-image-input="true">
    <label for="exampleInputImage" class="ms-5 form-label">{{ __('messages.vcard.profile_image') . ':' }}</label>
    <span data-bs-toggle="tooltip" data-placement="top" data-bs-original-title="{{ __('messages.tooltip.vcard_profile_img') }}">
      <i class="fas fa-question-circle ml-1 general-question-mark"></i>
    </span>
    <div class="d-block">
      <div class="image-picker ms-5">
        <div class="image previewImage" id="exampleInputImage" style="background-image: url('{{ !empty($vcard->profile_url) ? $vcard->profile_url : asset('web/media/avatars/user2.png') }}')">
        </div>
        <span class="picker-edit rounded-circle text-gray-500 fs-small" data-bs-toggle="tooltip" data-placement="top" data-bs-original-title="{{ __('messages.tooltip.profile') }}">
          <label>
            <i class="fa-solid fa-pen" id="profileImageIcon"></i>
            <input type="file" id="profile_image" name="profile_img" class="image-upload file-validation d-none crop-profile-input" accept="image/*" data-preview-id="vCardProfilePreview" />
          </label>
        </span>
      </div>
    </div>
...
<div class="col-lg-6 mb-7 favicon-input margin-top-40">
  <div class="mb-3 ms-4" io-image-input="true">
    ...
    <span data-bs-toggle="tooltip" data-placement="top" data-bs-original-title="{{ __('messages.tooltip.favicon_logo') }}">
      <i class="fas fa-question-circle ml-1 mt-1 general-question-mark"></i>
    </span>
    <div class="d-block">
      <div class="image-picker">
        <div class="image previewImage" id="exampleInputImage" style="background-image: url('{{ !empty($vcard->favicon_url) ? $vcard->favicon_url : $adminFavicon }}')">
        </div>
        <span class="picker-edit rounded-circle text-gray-500 fs-small" data-bs-toggle="tooltip" data-placement="top" data-bs-original-title="{{ __('messages.tooltip.change_favicon_logo') }}">
          <label>
            <i class="fa-solid fa-pen" id="faviconImageIcon"></i>
            <input type="file" id="favicon_image" name="favicon_img" class="image-upload file-validation d-none" accept="image/*" data-preview-id="vcardFaviconPreview" />
          </label>
        </span>
      </div>
    </div>
    ...
    <div class="row g-3 pt-5" id="basic">
      ...
      <div class="col-md-6 col-xl-3">
        <div class="card h-100 shadow p-5 text-center">
          ...
          <div class="form-check form-switch form-check-custom form-check-solid form-switch-sm d-flex justify-content-center">
      ...
      <div class="col-md-6 col-xl-3">
        <div class="card h-100 shadow p-5 text-center">
          <div class="d-flex justify-content-center align-items-center gap-1 mb-1">
            ...
            <span data-bs-toggle="tooltip" class="mb-3" data-placement="top" data-bs-original-title="{{ __('messages.tooltip.enable_affiliation') }}">
      ...
      <div class="col-md-6 col-xl-3">
        <div class="card h-100 shadow p-5 text-center">
          <div class="d-flex justify-content-center align-items-center gap-1 mb-1">
            ...
            <span data-bs-toggle="tooltip" class="mb-3" data-placement="top" data-bs-original-title="{{ __('messages.tooltip.enable_contact') }}">
              <i class="fas fa-question-circle ml-1 mx-1 general-question-mark"></i>
            </span>
          </div>
          <div class="form-check form-switch form-check-custom form-check-solid form-switch-sm d-flex justify-content-center">
      ...
      <div class="col-md-6 col-xl-3">
        <div class="card h-100 shadow p-5 text-center">
          <div class="d-flex justify-content-center align-items-center gap-1 mb-1">
            ...
            <span data-bs-toggle="tooltip" class="mb-3" data-placement="top" data-bs-original-title="{{ __('messages.tooltip.hide_stickybar') }}">
              <i class="fas fa-question-circle ml-1 mx-1 general-question-mark"></i>
            </span>
          </div>
          <div class="form-check form-switch form-check-custom form-check-solid form-switch-sm d-flex justify-content-center">
      ...
      <div class="col-md-6 col-xl-3">
        <div class="card h-100 shadow p-5 text-center">
          <div class="d-flex justify-content-center align-items-center gap-1 mb-1">
            ...
            <span data-bs-toggle="tooltip" class="mb-3" data-placement="top" data-bs-original-title="{{ __('messages.tooltip.whatsapp_share') }}">
              <i class="fas fa-question-circle ml-1 mx-1 general-question-mark"></i>
            </span>
          </div>
          <div class="form-check form-switch form-check-custom form-check-solid form-switch-sm d-flex justify-content-center">
      ...
      <div class="d-flex align-items-center">
        <input type="range" name="qr_code_download_size" class="form-range w-75 mx-2" value="{{ $vcard['qr_code_download_size'] }}" min="100" max="500" step="100" id="qrCodeDownloadSize" oninput="document.getElementById('download-result').innerText = this.value+'px'">
...
<div class="container-fluid">
  ...
  <div class="form-group mb-7 vcard-template">
    <div class="row">
      ...
      $folderName = basename(dirname($template->path));
      @endphp
      ...
      <div class="img-radio img-thumbnail {{ $id == 11 ? 'screen vcard_11' : '' }} {{ $vcard->template_id == $id ? 'img-border' : '' }} @if ($id == 22) ribbon @endif" data-id="{{ $id }}">
...
<div class="col-lg-12 mt-5 mb-5">
  <div class="form-check form-switch">
    <input class="form-check-input me-3" type="checkbox" id="vcardTemplateStatus" name="status" {{ $vcard->status ? 'checked' : '' }}>
    ...
    <div class="col-12 mb-5 d-flex align-items-center">
      ...
      <div class="me-4 d-flex align-items-center" style="min-width: 180px;">
        <div class="form-check form-switch me-2">
          <input class="form-check-input day-toggle" type="checkbox" id="dayToggle{{ $key }}" name="days[]" value="{{ $key }}" {{ $isChecked ? 'checked' : '' }}>
        </div>
      ...
      <div class="flex-grow-1">
        <div class="time-fields" id="timeFields{{ $key }}" style="{{ $isChecked ? 'display: flex;' : 'display: none;' }}">
          <div class="d-flex">
        ...
        <div class="closed-state" id="closedState{{ $key }}" style="{{ $isChecked ? 'display: none;' : 'display: flex;' }}">
          <div class="">
            <div class="">
...
<div class="col-12">
            ...
            <div class="d-flex w-100 align-items-center position-relative">
              <div class="d-flex row flex-md-row flex-column w-100 weekly-row">
                <div class="col-xl-2 form-check mb-5 d-flex align-items-center ms-5">
                  <input id="chkShortWeekDay_{{ $shortWeekDay }}" class="form-check-input" type="checkbox" value="{{ $day }}" name="checked_week_days[]" {{ !empty($time[$day]) ? 'checked' : '' }}>
              ...
              <div class="weekly-icon position-absolute end-0 d-flex">
                <a href="javascript:void(0)" class="add-session-time btn px-2 fs-2" id="add-session-{{ $day }}" data-day="{{ $day }}" data-bs-toggle="tooltip" title="{{ __('messages.common.add') }}">
                  <i class="fa fa-plus text-primary" aria-hidden="true"></i>
                </a>
                <div class="dropdown d-flex align-items-center">
                  <button class="btn dropdown-toggle copy-days-btn ps-2 pe-0 hide-arrow" type="button" id="dropdownCopyMenu-{{ $day }}" data-bs-toggle="dropdown" aria-expanded="false" data-bs-auto-close="outside">
                    <i class="fa-solid fa-copy text-primary fs-2"></i>
                  </button>
                  <div class="dropdown-menu copy-menu py-0 rounded-10 min-width-220" aria-labelledby="dropdownCopyMenu-{{ $day }}">
                    <div class="p-5 menu-content">
                      @foreach (App\Models\BusinessHour::WEEKDAY_NAME as $weekDayKey => $weekDay)
                      @if ($day != $weekDayKey)
                      <div class="mb-5 form-check ps-0 d-flex align-items-center justify-content-between copy-label">
                        <label class="form-check-label text-gray-900" for="chkCopyDay_{{ $shortWeekDay }}_{{ $weekDay }}">{{ ucfirst(__('messages.business.' . strtolower($weekDay))) }}</label>
                        <input type="checkbox" id="chkCopyDay_{{ $shortWeekDay }}_{{ $weekDay }}" class="form-check-input float-none copy-check-input ms-0" value="{{ $weekDayKey }}">
                      </div>
                      @endif
                      @endforeach
                      <button type="button" data-copy-day="{{ $day }}" class="btn btn-primary copy-btn w-100">{{ __('messages.appointment.copy') }}</button>
                    </div>
                  </div>
                </div>
              </div>
  ...
  <div class="d-flex align-items-center gap-4 py-3 flex-wrap">
    <div class="form-check me-4">
      <input class="form-check-input" type="radio" name="is_paid_radio" id="freeRadio" value="0" {{ isset($appointmentDetail) && $appointmentDetail->is_paid == 0 ? 'checked' : (!isset($appointmentDetail) ? 'checked' : '') }}>
    ...
    <div class="form-check d-flex align-items-center me-4">
      <input class="form-check-input me-2" type="radio" name="is_paid_radio" id="paidRadio" value="1" {{ isset($appointmentDetail) && $appointmentDetail->is_paid == 1 ? 'checked' : '' }}>
      ...
      <div class="d-flex">
        <div class="mb-3 mb-sm-0 me-3">
          <div class="" io-image-input="true">
            <div class="    ">
              <div class="image-picker">
                <div class="image previewImage " id="exampleInputImage" style="background-image: url('{!! $link->social_icon ?? 'https://cdn-icons-png.flaticon.com/512/87/87390.png' !!} ') ;width: 40px; height: 40px">
                </div>
                <span class="picker-edit rounded-circle text-gray-500 fs-small" data-bs-toggle="tooltip" data-placement="top" data-bs-original-title="{{ __('messages.tooltip.profile') }}" style="width: 22px; height: 22px">
                  <label>
                    <i class="fa-solid fa-pen" id="profileImageIcon"></i>
                    <input type="file" id="profile_image" name="social_links_image[{{ $key }}]" class="image-upload file-validation d-none social_links_image crop-image-input" accept="image/*" data-preview-id="socialLinkPreview{{ $key }}" value="{{ $link->social_icon }}" />
                  </label>
                </span>
              </div>
            </div>
        ...
        <div class="d-flex ml-2 w-100">
          <input type="text" class="form-control social_links" name="social_links[{{ $key }}]" value="{{ $link->link }}">
          <input type="hidden" name="social_link_id[{{ $key }}]" class="socialLinkId" value="{{ $link->id }}">
...
<div class="container-fluid">
  <div class="row">
    ...
    <div class="col-lg-6 mb-7">
      ...
      <div class="position-relative mb-3">
        <div class="mb-3 position-relative">
          <input class="form-control" type="password" placeholder="{{ __('messages.form.password') }}" value="{{ !empty($vcard->password) ? Crypt::decrypt($vcard->password) : '' }}" autocomplete="off" aria-label="Password" data-toggle="password" disabled />
    ...
    <div class="col-lg-6 mb-7">
      <div class="form-check form-switch">
        <input class="form-check-input" type="checkbox" id="branding" name="branding" {{ $vcard->branding ? 'checked' : '' }}>
        <label class="form-check-label" for="branding">
          {{ __('messages.vcard.remove_branding') }}
        </label>
        <span data-bs-toggle="tooltip" data-placement="top" data-bs-original-title="{{ __('messages.tooltip.remove_branding') }}">
          <i class="fas fa-question-circle ml-1 mt-1 general-question-mark"></i>
        </span>
      </div>
    </div>
...
<div class="container-fluid">
  <div class="row">
    <div class="col-lg-6 mb-7">
      ...
      {{ Form::select('font_family', \App\Models\Vcard::FONT_FAMILY, $vcard->font_family, [
            'class' => 'form-select',
  ...
  <div class="container-fluid">
        ...
        <div class="col-lg-12">
          <div class="mb-5">
            ...
            <div class="d-flex align-items-center mb-2">
              <a href="javascript:void(0)" id="generateAiPrivacyPolicy" class="text-primary text-decoration-none fw-semibold d-inline-flex align-items-center gap-2 me-3">
      ...
      <div class="row">
        <div class="col-lg-12">
          <div class="mb-5">
            ...
            <div class="d-flex align-items-center mb-2">
              <a href="javascript:void(0)" id="generateAiTermsConditions" class="text-primary text-decoration-none fw-semibold d-inline-flex align-items-center gap-2 me-3">
                <i class="bi bi-stars" id="termsConditionsIcon"></i>
                <span id="termsConditionsSpinner" class="spinner-border spinner-border-sm d-none" role="status"></span>
                {{ __('messages.vcard.write_with_ai') }}
              </a>
            </div>
      ...
      <div class="col-md-6">
        <div class="card shadow rounded-3">
          <div class="card-body">
            ...
            <div class="mb-4">
              <label for="exampleInputSelect2" class="form-label">{{ __('messages.vcard.qrcode_eye_style') }}</label>
              @php
            ...
            <div class="form-check form-switch mb-4">
              <input class="form-check-input me-3" name="applySetting" type="checkbox" id="flexSwitchCheckChecked" {{ isset($customQrCode['applySetting']) ? ($customQrCode['applySetting'] == 1 ? 'checked' : '') : '' }}>
              <label class="form-label" for="flexSwitchCheckChecked">{{ __('messages.common.use_this_configuration') }}</label>
            </div>
            <div class="col-lg-12 d-flex">
    ...
    <div class="container mt-5">
      <div class="card shadow-lg rounded-4 p-5">
        ...
        <div class="row mt-4">
          ...
          <div class="col-md-4 mb-5">
            <input id="contactListCheckbox" class="form-check-input me-2" type="checkbox" value="1" name="contact_list" {{ (isset($managesection) && $managesection['contact_list']) || empty($managesection) ? 'checked' : '' }}>
            <label class="form-check-label" for="contactListCheckbox">{!! __('messages.vcard.contact') !!}</label>
          </div>
          @if (checkFeature('services'))
          <div class="col-md-4 mb-5">
            <input id="servicesCheckbox" class="form-check-input me-2" type="checkbox" value="1" name="services" {{ (isset($managesection) && $managesection['services']) || empty($managesection) ? 'checked' : '' }}>
            <label class="form-check-label" for="servicesCheckbox">{!! __('messages.vcard.services') !!}</label>
          </div>
          @endif
          @if (checkFeature('gallery'))
          <div class="col-md-4 mb-5">
            <input id="galleriesCheckbox" class="form-check-input me-2" type="checkbox" value="1" name="galleries" {{ (isset($managesection) && $managesection['galleries']) || empty($managesection) ? 'checked' : '' }}>
            <label class="form-check-label" for="galleriesCheckbox">{!! __('messages.vcard.galleries') !!}</label>
          </div>
          @endif
          @if (checkFeature('products'))
          <div class="col-md-4 mb-5">
            <input id="productsCheckbox" class="form-check-input me-2" type="checkbox" value="1" name="products" {{ (isset($managesection) && $managesection['products']) || empty($managesection) ? 'checked' : '' }}>
            <label class="form-check-label" for="productsCheckbox">{!! __('messages.vcard.products') !!}</label>
          </div>
          @endif
          @if (checkFeature('testimonials'))
          <div class="col-md-4 mb-5">
            <input id="testimonialsCheckbox" class="form-check-input me-2" type="checkbox" value="1" name="testimonials" {{ (isset($managesection) && $managesection['testimonials']) || empty($managesection) ? 'checked' : '' }}>
            <label class="form-check-label" for="testimonialsCheckbox">{!! __('messages.vcard.testimonials') !!}</label>
          </div>
          @endif
          @if (checkFeature('blog'))
          <div class="col-md-4 mb-5">
            <input id="blogsCheckbox" class="form-check-input me-2" type="checkbox" value="1" name="blogs" {{ (isset($managesection) && $managesection['blogs']) || empty($managesection) ? 'checked' : '' }}>
            <label class="form-check-label" for="blogsCheckbox">{!! __('messages.vcard.blogs') !!}</label>
          </div>
          @endif
          <div class="col-md-4 mb-5">
            <input id="businessHoursCheckbox" class="form-check-input me-2" type="checkbox" value="1" name="business_hours" {{ (isset($managesection) && $managesection['business_hours']) || empty($managesection) ? 'checked' : '' }}>
          ...
          <div class="col-md-4 mb-5">
            <input id="appointmentsCheckbox" class="form-check-input me-2" type="checkbox" value="1" name="appointments" {{ (isset($managesection) && $managesection['appointments']) || empty($managesection) ? 'checked' : '' }}>
            <label class="form-check-label" for="appointmentsCheckbox">{!! __('messages.vcard.appointments') !!}</label>
          </div>
          @endif
          <div class="col-md-4 mb-5">
            <input id="mapCheckbox" class="form-check-input me-2" type="checkbox" value="1" name="map" {{ (isset($managesection) && $managesection['map']) || empty($managesection) ? 'checked' : '' }}>
            <label class="form-check-label" for="mapCheckbox">{!! __('messages.vcard.map') !!}</label>
          </div>
          <div class="col-md-4 mb-5">
            <input id="bannerCheckbox" class="form-check-input me-2" type="checkbox" value="1" name="banner" {{ (isset($managesection) && $managesection['banner']) || empty($managesection) ? 'checked' : '' }}>
            <label class="form-check-label" for="bannerCheckbox">{!! __('messages.front_cms.banner_title') !!}</label>
          </div>
          @if (checkFeature('insta_embed'))
          <div class="col-md-4 mb-5">
            <input id="instaembedCheckbox" class="form-check-input me-2" type="checkbox" value="1" name="insta_embed" {{ (isset($managesection) && $managesection['insta_embed']) || empty($managesection) ? 'checked' : '' }}>
            <label class="form-check-label" for="instaembedCheckbox">{!! __('messages.feature.insta_embed') !!}</label>
          </div>
          @endif
          @if (checkFeature('linkedin_embed'))
          <div class="col-md-4 mb-5">
            <input id="linkedinembedCheckbox" class="form-check-input me-2" type="checkbox" value="1" name="linkedin_embed" {{ (isset($managesection) && $managesection['linkedin_embed']) || empty($managesection) ? 'checked' : '' }}>
            <label class="form-check-label" for="linkedinembedCheckbox">{!! __('messages.feature.linkedin_embed') !!}</label>
          </div>
          @endif
          @if (checkFeature('iframes'))
          <div class="col-md-4 mb-5">
            <input id="iframeCheckbox" class="form-check-input me-2" type="checkbox" value="1" name="iframe" {{ (isset($managesection) && $managesection['iframe']) || empty($managesection) ? 'checked' : '' }}>
            <label class="form-check-label" for="iframeCheckbox">{!! __('messages.vcard.iframe') !!}</label>
          </div>
          @endif
          <div class="col-md-4 mb-5">
            <input id="newsLatterPopupCheckbox" class="form-check-input me-2" type="checkbox" value="1" name="news_latter_popup" {{ (isset($managesection) && $managesection['news_latter_popup']) || empty($managesection) ? 'checked' : '' }}>
            ...
            <div class="col text-md-start text-center">
              ...
              <div class="col">
                <label class="button-label mx-2 mb-3">
                  <input type="radio" name="sticky_bar" class="sticky-btn btn btn-secondary" value="0" {{ isset($dynamicVcard['sticky_bar']) && $dynamicVcard['sticky_bar'] == 0 ? 'checked' : '' }}>
                  <span class="sticky-btn-title">{{ __('messages.vcard.left') }}</span>
                </label>
                <label class="button-label mx-2">
                  <input type="radio" name="sticky_bar" class="sticky-btn btn btn-secondary" value="1" {{ empty($dynamicVcard) || $dynamicVcard['sticky_bar'] == 1 ? 'checked' : '' }}>
    ...
    <script>
      $(document).ready(function() {
        ...
        // ── URL Alias smart validation ──────────────────────────────────────────
        var aliasExtractMsg = @json(__('messages.vcard.url_alias_full_url_extracted'));

        function cleanAlias(raw) {
          raw = raw.trim();
          // If it looks like a full URL, pull out the last non-empty path segment
          if (/^https?:\/\//i.test(raw) || raw.indexOf('/') !== -1) {
            try {
              // Make it parseable even without a scheme
              var url = raw.match(/^https?:\/\//i) ? raw : 'https://' + raw;
              ...
              // fallback: grab last slash segment
              var parts = raw.split('/').filter(function(s) {
                return s.length > 0;
              });
        ...
        $('#vcard-url-alias').on('input paste', function() {
          var $input = $(this);
          // Small delay to let paste populate the value
          setTimeout(function() {
            var val = $input.val();
            var cleaned = cleanAlias(val);
            var $extractMsg = $('#url-alias-extract-msg');
            var $invalidMsg = $('#url-alias-invalid-msg');
            $extractMsg.addClass('d-none');
            $invalidMsg.addClass('d-none');

            if (cleaned !== val && cleaned.length > 0) {
              // Auto-fix full URL → alias only
              $input.val(cleaned);
              $('#url-alias-extract-text').text(aliasExtractMsg + ' ' + cleaned);
              $extractMsg.removeClass('d-none');
              val = cleaned;
            }

            // Validate characters: letters, digits, hyphens, underscores only
            if (val.length > 0 && !/^[A-Za-z0-9_-]+$/.test(val)) {
              $invalidMsg.removeClass('d-none');
            }

            // Auto-fill hidden name field from alias
            if (val.length > 0) {
              $('#vcard-name-hidden').val(val);
            }
          }, 50);
        });
        // ───────────────────────────────────────────────────────────────────────
