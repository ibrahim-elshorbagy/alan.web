        ...
        <div class="row">
          <div class="col-lg-6 col-sm-8 mb-7">
            <div class="form-group mb-7">
              ...
              @endphp
              {{ Form::select('cover_type', $coverType, isset($vcard) ? $vcard->cover_type : null, ['class' =>
              'form-select cover-type', 'id' => 'coverType', 'data-control' => 'select2']) }}
            </div>
                ...
                <div class="mb-3">
                  <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="cover_image_source" id="predefinedCover"
                      value="predefined" {{ !isset($vcard) || (!empty($vcard->cover_url) &&
                    !str_contains($vcard->cover_url, 'cover_images')) ? 'checked' : '' }}>
                  ...
                  <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="cover_image_source" id="customCover" value="custom"
                      {{ isset($vcard) && !empty($vcard->cover_url) && str_contains($vcard->cover_url, 'cover_images') ?
                    'checked' : '' }}>
            ...
            <div class="col-lg-6 col-sm-6 mb-7 cover-video d-none ms-5">
              <div class="mb-3" io-image-input="true">
                ...
                <div class="d-block">
                  <div class="images-picker">
                    <div class="image previewImage" id="coverPreview">
                      @if (!empty($vcard->cover_url) && in_array(pathinfo($vcard->cover_url, PATHINFO_EXTENSION), ['mp4',
                      'mov', 'avi']))
            ...
            <div class="col-lg-12 mb-7 d-none cover_youtube_link mt-3">
              {{ Form::label('youtube_link', __('messages.cover_type.youtube_link') . ':', ['class' => 'form-label fs-6
              text-gray-700 mb-3']) }}
              {{ Form::text('youtube_link', isset($vcard) ? $vcard->youtube_link : null, ['class' => 'form-control',
              'placeholder' => 'https://www.youtube.com/watch?v=hAGbufevHM4', 'id' => '']) }}
            </div>

            <div class="col-lg-12 mb-7 mt-3">
              <div class="form-group">
                {{ Form::label('cover_image_type', __('messages.cover_image_type.cover_image_type') . ':', ['class' =>
                'form-label']) }}
                @php
                ...
                @endphp
                {{ Form::select('cover_image_type', $coverImageType, isset($vcard) ? $vcard->cover_image_type : null,
                ['class' => 'form-select', 'id' => 'cover_image_type', 'data-control' => 'select2']) }}
              </div>
            </div>
          ...
          <div class="col-lg-6 col-sm-4 mb-7">
            <div class="mb-3 ms-5" io-image-input="true">
              <label for="exampleInputImage" class="ms-5 form-label">{{ __('messages.vcard.profile_image') . ':'
                }}</label>
              <span data-bs-toggle="tooltip" data-placement="top"
                ...
                <div class="image-picker">
                  ...
                  <span class="picker-edit rounded-circle text-gray-500 fs-small" data-bs-toggle="tooltip"
                    ...
                    <label>
                      ...
                      <input type="file" id="favicon_image" name="favicon_img" class="image-upload file-validation d-none"
                        accept="image/*" data-preview-id="vcardFaviconPreview" />
                    </label>
                  </span>
                </div>
              </div>
            ...
            <div class="card shadow p-4">
              ...
              <div class="d-flex align-items-center">
                <input type="range" name="qr_code_download_size" class="form-range w-75 mx-2"
                  value="{{ $vcard['qr_code_download_size'] }}" min="100" max="500" step="100" id="qrCodeDownloadSize"
...
@if ($partName === 'business-hours')
  <style>
    ...
    .bh-day-row:hover {
      box-shadow: 0 2px 12px rgba(108, 132, 245, .12);
      border-color: #c5cef7;
    }
    ...
    .bh-copy-btn.btn-success {
      animation: bhFlash .5s;
    }
  ...
  <div class="container-fluid">
    <div class="row">
      ...
      {{-- Days rendered in default order; JS reorders live on week_format change --}}
      <div id="bh-days-container" class="col-12 px-0">
        @foreach (\App\Models\BusinessHour::DAY_OF_WEEK as $key => $day)
          ...
          <div class="col-12 mb-3 bh-day-row d-flex align-items-center gap-3 flex-wrap" data-day-key="{{ $key }}">

            <div class="d-flex align-items-center" style="min-width: 160px;">
              <div class="form-check form-switch me-2 mb-0">
                <input class="form-check-input day-toggle" type="checkbox" id="dayToggle{{ $key }}" name="days[]"
                  value="{{ $key }}" {{ $isChecked ? 'checked' : '' }}>
              </div>
              <label class="form-check-label fw-semibold mb-0 text-gray-700" for="dayToggle{{ $key }}">
            ...
            <div class="flex-grow-1">
              <div class="time-fields" id="timeFields{{ $key }}"
                style="{{ $isChecked ? 'display: flex;' : 'display: none;' }} align-items: center; gap: 6px; flex-wrap: wrap;">
                <div class="d-flex align-items-center gap-1">
                  <span class="input-group-text bg-light text-muted small py-1 px-2">{{ __('messages.common.from') }}</span>
                  {{ Form::select("startTime[$key]", getSchedulesTimingSlot(), $start, ['class' => 'form-control form-control-sm bh-select-mini bh-start', 'data-control' => 'select2']) }}
                </div>
                <div class="d-flex align-items-center gap-1">
                  <span class="input-group-text bg-light text-muted small py-1 px-2">{{ __('messages.common.to') }}</span>
                  {{ Form::select("endTime[$key]", getSchedulesTimingSlot(), $end, ['class' => 'form-control form-control-sm bh-select-mini bh-end', 'data-control' => 'select2']) }}
                </div>
                <button type="button" class="btn btn-outline-primary btn-sm bh-copy-btn ms-2" data-source-key="{{ $key }}"
                  title="{{ __('messages.business.copy_to_all') }}">
                  <i class="fas fa-copy me-1"></i>{{ __('messages.business.copy_to_all') }}
                </button>
              </div>

              <div class="closed-state" id="closedState{{ $key }}"
                style="{{ $isChecked ? 'display: none;' : 'display: flex;' }} align-items: center;">
                <span class="badge bg-light text-muted border px-3 py-2" style="font-size:.8rem; border-radius:20px;">
                  <i class="fas fa-moon me-1 text-secondary"></i>{{ __('messages.common.closed') }}
                </span>
              </div>
            </div>
          </div>
  ...
  <script>
    (function () {
      /* ── Day order maps (mirrors PHP BusinessHour::WEEK_FORMAT_TYPE) ── */
      var ORDER_MAP = {
        '1': ['1', '2', '3', '4', '5', '6', '7'], // Mon → Sun
        '2': ['7', '1', '2', '3', '4', '5', '6'], // Sun → Sat
        '3': ['6', '7', '1', '2', '3', '4', '5']  // Sat → Fri
      };

      function reorderDays(formatVal) {
        var order = ORDER_MAP[String(formatVal)] || ORDER_MAP['1'];
        var container = document.getElementById('bh-days-container');
        if (!container) return;
        order.forEach(function (key) {
          var row = container.querySelector('[data-day-key="' + key + '"]');
          if (row) container.appendChild(row); // move to end = sorted order
        });
      }

      /* Apply initial order from the current select value */
      var weekFormatSelect = document.getElementById('week_format');
      if (weekFormatSelect) {
        reorderDays(weekFormatSelect.value);

        /* Native change (works with plain <select>) */
        weekFormatSelect.addEventListener('change', function () {
          reorderDays(this.value);
        });

        /* Select2 change (fires after Select2 updates the value) */
        if (window.$) {
          $(weekFormatSelect).on('select2:select', function (e) {
            reorderDays(e.params.data.id);
          });
        }
      }
      ...
      function enableDay(key) {
        var toggle = document.getElementById('dayToggle' + key);
        if (!toggle || toggle.checked) return;
        toggle.checked = true;
        toggle.dispatchEvent(new Event('change', { bubbles: true }));
        var tf = document.getElementById('timeFields' + key);
        var cs = document.getElementById('closedState' + key);
        if (tf) tf.style.display = 'flex';
        if (cs) cs.style.display = 'none';
      }

      var allDayKeys = Array.from(document.querySelectorAll('[data-day-key]'))
        .map(function (el) { return el.getAttribute('data-day-key'); });

      /* ── per-row "Copy to all" ── */
      document.addEventListener('click', function (e) {
        var btn = e.target.closest('.bh-copy-btn');
        if (!btn) return;
        var sourceKey = btn.getAttribute('data-source-key');
        var row = document.querySelector('[data-day-key="' + sourceKey + '"]');
        if (!row) return;
        var startVal = row.querySelector('.bh-start').value;
        var endVal = row.querySelector('.bh-end').value;
        allDayKeys.forEach(function (key) {
          if (key === sourceKey) return;
          enableDay(key);
          var r = document.querySelector('[data-day-key="' + key + '"]');
          if (!r) return;
          setSelectValue(r.querySelector('.bh-start'), startVal);
          setSelectValue(r.querySelector('.bh-end'), endVal);
        });
...
@if ($partName == 'appointments')
  <div class="col-12">
              ...
              <div class="weekly-content" data-day="{{ $day }}">
                <div class="d-flex w-100 align-items-center position-relative">
                  <div class="d-flex row flex-md-row flex-column w-100 weekly-row">
                    <div class="col-xl-2 form-check mb-5 d-flex align-items-center ms-5">
                      <input id="chkShortWeekDay_{{ $shortWeekDay }}" class="form-check-input" type="checkbox"
                        value="{{ $day }}" name="checked_week_days[]" {{ !empty($time[$day]) ? 'checked' : '' }}>
                  ...
                  <div class="weekly-icon position-absolute end-0 d-flex">
                    <a href="javascript:void(0)" class="add-session-time btn px-2 fs-2" id="add-session-{{ $day }}"
                    ...
                    <div class="dropdown d-flex align-items-center">
                      <button class="btn dropdown-toggle copy-days-btn ps-2 pe-0 hide-arrow" type="button"
                        id="dropdownCopyMenu-{{ $day }}" data-bs-toggle="dropdown" aria-expanded="false"
                          ...
                          @foreach (App\Models\BusinessHour::WEEKDAY_NAME as $weekDayKey => $weekDay)
                            @if ($day != $weekDayKey)
                              <div class="mb-5 form-check ps-0 d-flex align-items-center justify-content-between copy-label">
                                ...
                                <input type="checkbox" id="chkCopyDay_{{ $shortWeekDay }}_{{ $weekDay }}"
                                  class="form-check-input float-none copy-check-input ms-0" value="{{ $weekDayKey }}">
                              </div>
                            @endif
                          @endforeach
    ...
    <input type="hidden" id="isUserPaidId" name="is_paid"
      value="{{ isset($appointmentDetail->is_paid) ? $appointmentDetail->is_paid : 0 }}">
    <div class="d-flex align-items-center gap-4 py-3 flex-wrap">
      ...
      <div class="form-check d-flex align-items-center me-4">
        ...
        <div id="userPaidInputDiv"
          class="{{ isset($appointmentDetail->is_paid) && $appointmentDetail->is_paid == 1 ? '' : 'd-none' }}">
                ...
                <div class="    ">
                  <div class="image-picker">
                    ...
                    <span class="picker-edit rounded-circle text-gray-500 fs-small" data-bs-toggle="tooltip"
                      ...
                      <label>
                        <i class="fa-solid fa-pen" id="profileImageIcon"></i>
                        <input type="file" id="profile_image" name="social_links_image[{{ $key }}]"
                          class="image-upload file-validation d-none social_links_image crop-image-input" accept="image/*"
                          data-preview-id="socialLinkPreview{{ $key }}" value="{{ $link->social_icon }}" />
                      </label>
                    </span>
                  </div>
                </div>
        ...
        <div class="card shadow-lg rounded-4 p-5">
          ...
          <div class="row mt-4">
            <div class="col-md-4 mb-5">
              <input id="headerCheckbox" class="form-check-input me-2" type="checkbox" value="1" name="header" checked
                disabled>
            ...
            <div class="col-md-4 mb-5">
              <input id="businessHoursCheckbox" class="form-check-input me-2" type="checkbox" value="1"
                name="business_hours" {{ (isset($managesection) && $managesection['business_hours']) || empty($managesection) ? 'checked' : '' }}>
              <label class="form-check-label"
                for="businessHoursCheckbox">{!! __('messages.vcard.business_hours') !!}</label>
            </div>
            ...
            @if (checkFeature('linkedin_embed'))
              <div class="col-md-4 mb-5">
                <input id="linkedinembedCheckbox" class="form-check-input me-2" type="checkbox" value="1"
                  name="linkedin_embed" {{ (isset($managesection) && $managesection['linkedin_embed']) || empty($managesection) ? 'checked' : '' }}>
                <label class="form-check-label"
                  for="linkedinembedCheckbox">{!! __('messages.feature.linkedin_embed') !!}</label>
              </div>
            @endif
            ...
            @endif
            <div class="col-md-4 mb-5">
              <input id="newsLatterPopupCheckbox" class="form-check-input me-2" type="checkbox" value="1"
                name="news_latter_popup" {{ (isset($managesection) && $managesection['news_latter_popup']) || empty($managesection) ? 'checked' : '' }}>
              <label class="form-check-label"
                for="newsLatterPopupCheckbox">{!! __('messages.vcard.newslatter_popup') !!}</label>
            </div>
          </div>
