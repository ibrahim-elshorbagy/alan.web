<header class='d-flex align-items-center justify-content-between flex-grow-1 header px-3 px-xl-0'>
  ...
  <nav
    class="navbar navbar-expand-xl navbar-light top-navbar d-xl-flex d-block px-3 px-xl-0 py-4 py-xl-0 {{ !getLogInUser()->theme_mode ? 'bg-white' : '' }}"id="nav-header">
    <div class="container-fluid pe-0">
      <div class="navbar-collapse">
        <ul
          class="navbar-nav me-auto mb-2 mb-lg-0 {{ !Request::is(
              'sadmin/front-cms*',
  ...
  <ul class="nav align-items-center flex-nowrap">
                ...
                @if (array_key_exists($language->iso_code, \App\Models\User::FLAG))
                  @foreach (\App\Models\User::FLAG as $imageKey => $imageValue)
                    @if ($imageKey == $language->iso_code)
                      <img src="{{ asset($imageValue) }}" class="me-2" style="width: 24px; height: 28px;" />
                    @endif
                  @endforeach
                @else
                  @if (count($language->media) != 0)
                    <img src="{{ $language->image_url }}" class="me-2" style="width: 24px; height: 28px;" />
                  @else
    ...
    @impersonating
      <li class="px-xxl-3 px-2">
        <span class="text-primary">
          <a data-turbo="false" data-turbo-eval="false" class="dashboard-theme-icons"
      ...
      <div class="dropdown d-flex align-items-center py-4">
        ...
        <div class="dropdown-menu py-4 my-2" aria-labelledby="dropdownMenuButton1" data-bs-auto-close="outside"
          ...
          <ul class="pt-4">
            ...
            @endrole
            <li class="px-4">
              <a class="dropdown-item  dashboard-dropdown text-gray-900 d-flex justify-content-start"
                href="javascript:void(0)">
                <span class="dropdown-icon me-2">
