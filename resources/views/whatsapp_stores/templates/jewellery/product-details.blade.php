...
<body>
  <div class="main-content mx-auto w-100 overflow-hidden d-flex flex-column justify-content-between"
        ...
        <div class="container-fluid header">
          ...
          <div class="d-flex align-items-center gap-lg-4 gap-sm-3 gap-2">
            ...
            <button class="add-to-cart-btn d-flex align-items-center justify-content-center position-relative"
              id="addToCartViewBtn">
              <div
                class="position-absolute cart-count d-flex align-items-center justify-content-center count-product product-count-badge badge rounded-pill bg-danger">
              </div>
              ...
              <div class="details d-flex flex-column justify-content-between h-100">
                <div>
                  ...
                  <p class="fs-28 fw-5 mb-30">
                    @if ($product->net_price)
                      <del
                        class="fs-18 fw-5 text-gray-200 pe-2">{{ $product->currency->currency_icon }}{{ $product->net_price }}</del>
                    @endif
                    <span
                      class="currency_icon selling_price">{{ currencyFormat($product->selling_price, 2, $product->currency->currency_code) }}</span>
                    @if ($product->available_stock == 0)
                      <span
                        class="badge badge-danger bg-danger text-sm out-of-stock-text ms-2">{{ __('messages.whatsapp_stores.out_of_stock') }}</span>
                    @endif
                  </p>
                ...
                @if ($product->available_stock != 0)
                  <div class="d-flex gap-2 align-items-center flex-wrap flex-sm-nowrap">
                    <button
                      class="btn add-to-cart px-2 d-flex align-items-center justify-content-center gap-20 fs-20 fw-5 position-relative addToCartBtn"
                      data-id="{{ $product->id }}">
          ...
          @foreach ($whatsappStore->products as $index => $product)
            ...
            <div>
              <a href="{{ route('whatsapp.store.product.details', [$whatsappStore->url_alias, $product->id]) }}"
                class="product-card h-100 d-flex flex-column {{ $cardClass }}">
                ...
                <div class="product-desc">
                  ...
                  <div class="d-flex justify-content-between align-items-center">
                    <h4 class="fs-22 text-black fw-6 mb-0">
                      ...
                      @if ($product->net_price)
                        <del
                          class="fs-14 fw-5 text-gray-200 ms-2">{{ $product->currency->currency_icon }}{{ $product->net_price }}</del>
                      @endif
                    </h4>
                    @if ($product->available_stock > 0)
                    ...
                    @else
                      <span
                        class="badge bg-danger text-white">{{ __('messages.whatsapp_stores.out_of_stock') }}</span>
                    @endif
                  </div>
                </div>
              </a>
    ...
    <div class="position-absolute items-detials-vector-1 vector-all text-center">
      <img src="{{ asset('assets/img/whatsapp_stores/jewellery/product-items-details-vector-1.png') }}"
        alt="imagees" />
    </div>
    <div class="position-absolute items-detials-vector-1 vector-all text-center">
      <img src="{{ asset('assets/img/whatsapp_stores/jewellery/product-items-details-vector-2.png') }}"
        alt="imagees" />
    </div>
  </div>
</body>
