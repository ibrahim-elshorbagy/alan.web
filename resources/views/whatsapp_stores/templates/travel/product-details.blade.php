  ...
  <div class="main-content" @if (getLanguage($whatsappStore->default_language) == 'Arabic' ||
        ...
        <div class="d-flex align-items-center">
          ...
          <span class="d-none d-sm-block">
            <a href="{{ route('whatsapp.store.show', $whatsappStore->url_alias) }}"
              class="text-decoration-none primary-text fs-18 fw-bold primary-text text-uppercase">{{ $whatsappStore->store_name }}</a>
          </span>
        </div>
        <div class="d-flex align-items-center">
          ...
          <button
            ...
            data-bs-toggle="modal" data-bs-target="#cartModal" id="addToCartViewBtn">
              ...
              <p class="fs-28 fw-5 mt-2 d-flex align-items-center flex-wrap gap-2">
                @if ($product->net_price)
                  <span class="fs-14 fw-semibold">
                    <del
                      class="fs-18 fw-5 text-gray pe-2">{{ currencyFormat($product->net_price, 2, $product->currency->currency_code) }}</del>
                  </span>
                @endif
                <span class="badge rounded-pill px-3 py-2 fs-16"
                  style="background-color: #faeee6; color: #dc834e; border: 1px solid #dc834e;">
                  <span
                    class="currency_icon selling_price">{{ currencyFormat($product->selling_price, 2, $product->currency->currency_code) }}</span>
                </span>
                @if ($product->available_stock == 0)
                  <span
                    class="badge badge-danger bg-danger text-sm out-of-stock-text">{{ __('messages.whatsapp_stores.out_of_stock') }}</span>
                @endif
              </p>
            </div>
          </div>
            ...
            <div class="d-flex justify-content-end">
              <div class="button-primary-small">
                <button class="addToCartBtn"
                  data-id="{{ $product->id }}">{{ __('messages.whatsapp_stores_templates.book_now') }}</button>
              </div>
            </div>
          @endif
        </div>
    ...
    <section class="product-description">
      <div>
        <div class="d-flex flex-column gap-20 mb-4 pb-1">
          <h5 class="fs-20 fw-semibold leading-104 text-black">
            {{ __('messages.whatsapp_stores_templates.description') }}</h5>
    ...
    <section class="product-section position-relative">
      <h2 class="font-aclonica text-center">
        <span class="font-dancing">{{ __('messages.whatsapp_stores_templates.recommended') }} </span>
        <br />{{ __('messages.whatsapp_stores.package') }}
      </h2>
            ...
            <div>
              <div class="product-card {{ $cardClass }}">
                <div class="product-img">
                  <img src="{{ $product->images_url[0] ?? '' }}" alt="img"
                    class="w-100 h-100 object-fit-cover" />
                  <div
                    ...
                    <a href="{{ route('whatsapp.store.product.details', [$whatsappStore->url_alias, $product->id]) }}"
                      class="fs-20 fw-semibold         primary-text">Explore</a>
                    <span>
                ...
                <div class="card-content">
                  ...
                  <div class="card-time button-primary-small">
                    @if ($product->available_stock > 0)
                      <button class="d-flex align-items-center justify-content-between addToCartBtn "
                        data-id="{{ $product->id }}">
                        {{ __('messages.whatsapp_stores_templates.book_now') }}
                      </button>
                    @else
                      <button
                        class="badge bg-danger text-white fs-10 float-end disabled">{{ __('messages.whatsapp_stores.out_of_stock') }}</button>
                    @endif
                  </div>
                </div>
              </div>
