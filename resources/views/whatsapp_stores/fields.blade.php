  ...
  <div class="row" id="basic">
    ...
    <div class="col-lg-6 pt-7">
      <div class="card shadow p-4">
        ...
        <div class="d-flex align-items-center">
          <input type="range" name="qr_code_download_size" class="form-range w-75 mx-2" value="{{ $whatsappStore->qr_code_download_size ?? 200 }}" min="100" max="500" step="100" id="qrCodeDownloadSize" oninput="document.getElementById('download-result').innerText = this.value+'px'">
    ...
    <div class="col-lg-3">
      <div class="form-check  mb-7">
        <input id="newsLetterPopupCheckbox" class="form-check-input me-2" type="checkbox" value="1" name="news_letter_popup" {{ isset($whatsappStore->news_letter_popup) && $whatsappStore->news_letter_popup ? 'checked' : '' }}>
    ...
    <div class="col-lg-3">
      <div class="form-check  mb-7">
        <input id="businessHoursCheckbox" class="form-check-input me-2" type="checkbox" value="1" name="business_hours" {{ isset($whatsappStore->business_hours) && $whatsappStore->business_hours ? 'checked' : '' }}>
    ...
    <div class="col-lg-3">
      <div class="form-check  mb-7">
        <input id="enableDownloadQrCheckbox" class="form-check-input me-2" type="checkbox" value="1" name="enable_download_qr_code" {{ isset($whatsappStore->enable_download_qr_code) && $whatsappStore->enable_download_qr_code ? 'checked' : '' }}>
    ...
    <div class="col-lg-3">
      <div class="form-check  mb-7">
        <input id="hideStickybarCheckbox" class="form-check-input me-2" type="checkbox" value="1" name="hide_stickybar" {{ isset($whatsappStore->hide_stickybar) && $whatsappStore->hide_stickybar ? 'checked' : '' }}>
    ...
    <div id="customUpload" style="{{ (isset($whatsappStore) && !empty($whatsappStore->cover_url) && !str_contains($whatsappStore->cover_url, 'cover_images')) ? '' : 'display: none;' }}">
      <div class="d-block">
        <div class="images-picker">
          <div class="image previewImage" id="coverPreview" style="background-image: url('{{ !empty($whatsappStore->cover_url) ? $whatsappStore->cover_url : '' }}');">
          </div>
          <span class="picker-edit rounded-circle text-gray-500 fs-small" data-bs-toggle="tooltip" data-placement="top" data-bs-original-title="{{ __('messages.tooltip.cover') }}">
            <label>
              ...
              <input type="file" id="coverImg" name="cover_img" class="d-none" accept="image/*, video/*" data-preview-id="whatsappStoreCoverPreview" />
            </label>
          </span>
        </div>
      </div>
  ...
  <div class="mb-3" io-image-input="true">
    ...
    <div class="d-block">
      <div class="image-picker">
        <div class="image previewImage" id="exampleInputImage" style="background-image: url('{{ !empty($whatsappStore->logo_url) ? $whatsappStore->logo_url : '' }}')">
        </div>
        <span class="picker-edit rounded-circle text-gray-500 fs-small" data-bs-toggle="tooltip" data-placement="top" data-bs-original-title="{{ __('messages.whatsapp_stores.change_logo') }}">
          <label>
            <i class="fa-solid fa-pen"></i>
            <input type="file" id="logo" name="logo" class="image-upload d-none" accept="image/*" data-preview-id="whatsappStoreLogoPreview" />
          </label>
        </span>
      </div>
    </div>
    <div data-image-paste data-file-input-id="logo" data-preview-id="exampleInputImage" data-button-text="{{ __('messages.select_image') }}" data-clipboard-button-text="{{ __('messages.paste_from_clipboard') }}" data-success-text="{{ __('messages.image_pasted_successfully') }}" data-invalid-type-text="{{ __('messages.invalid_image_type') }}" data-image-too-large-text="{{ __('messages.image_too_large') }}" data-no-image-text="{{ __('messages.no_image_in_clipboard') }}">
    </div>
  </div>
  <div class="form-text">{{ __('messages.allowed_file_types') }}</div>
</div>
...
<div class="container-fluid">
  ...
  <div class="row">
    ...
    <div class="col-12 col-md-6">
      <div class="form-group mb-7">
        <input type="hidden" name="template_id" id="themeInput" value="{{ $whatsappStore->template_id }}" id="themeInput">
        <div class="theme-img-radio img-thumbnail {{ $whatsappStore->template_id == $id ? 'img-border' : '' }}" data-id="{{ $id }}">
...
<div class="container-fluid">
  <div class="row">
    ...
    <div class="col-12 mb-5 d-flex align-items-center flex-wrap gap-3">

      <div class="me-4 d-flex align-items-center" style="min-width: 180px; flex: 1; align-items: flex-start;">
      ...
      <div class="flex-grow-1 w-100">
        <div class="time-fields" id="timeFields{{ $key }}" style="{{ $isChecked ? 'display: flex;' : 'display: none;' }}">
          <div class="d-flex flex-wrap gap-2">
            ...
            {{ Form::select("startTime[$key]", getSchedulesTimingSlot(), $start, ['class' => 'form-control w-100 mb-2 mb-md-0', 'data-control' => 'select2']) }}

            <span class="input-group-text bg-light text-muted small ">{{ __('messages.common.to') }}</span>
            {{ Form::select("endTime[$key]", getSchedulesTimingSlot(), $end, ['class' => 'form-control w-100 mb-2 mb-md-0', 'data-control' => 'select2']) }}
          </div>
        </div>
  ...
  <div class="tab-pane fade show active" id="overview" role="tabpanel" aria-labelledby="overview-tab">
    <div class="row">
      <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
        <label for="name" class="pb-2 fs-4 text-gray-600">{{ __('messages.whatsapp_stores.order_id') }}:</label>
      ...
      <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
        ...
        <span class="fs-4 text-gray-800" dir="ltr" style='{{ getCurrentLanguageName() == 'ar' || getCurrentLanguageName() == 'fa' ? 'margin-right: 0px; margin-left: auto;"' : '' }}'>+{{ $wpOrder->region_code }}
          {{ $wpOrder->phone }}</span>
      </div>
...
<div class="container-fluid">
  ...
  <div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
      ...
      <div>
        <button type="button" class="btn btn-success youtube-links" data-whatsapp-store-id="{{ $whatsappStore->id }}">
    ...
    <div class="row youtube-links-add">
      ...
      <div class="col-lg-6 mb-3 youtube-links-div">
        <div class="d-flex">
          <div class="d-flex w-100">
            <input type="text" class="form-control youtube_links" name="youtube_links[{{ $key }}]" value="{{ $video->youtube_link }}" placeholder="{{ __('messages.whatsapp_stores.enter_youtube_video_link') }}">
            <input type="hidden" name="youtube_link_id[{{ $key }}]" class="youtubeLinkId" value="{{ $video->id }}">
            <a href="javascript:void(0)" title="{{ __('messages.common.delete') }}" class="btn px-1 text-danger fs-3 youtube-links-delete-btn">
      ...
      <div class="col-lg-6 mb-3 youtube-links-div">
        <div class="d-flex">
          <div class="d-flex w-100">
            <input type="text" class="form-control youtube_links" name="youtube_links[]" placeholder="{{ __('messages.whatsapp_stores.enter_youtube_video_link') }}">
    ...
    <div class="mt-3">
      <button type="button" class="btn btn-primary youtube_link_save" data-whatsapp-store-id="{{ $whatsappStore->id }}">
        {{ __('messages.common.save') }}
      </button>
    </div>
  </div>
