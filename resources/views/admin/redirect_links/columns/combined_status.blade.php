...
<div class="d-flex justify-content-center align-items-center gap-2">
  ...
  @if ($isClickable)
  <i class="{{ $receivedIcon }} cursor-pointer" data-bs-toggle="tooltip" title="{{ $receivedTooltip }}" wire:click="markAsReceived({{ $row->id }})"></i>
  @else
  <i class="{{ $receivedIcon }}" data-bs-toggle="tooltip" title="{{ $receivedTooltip }}"></i>
  @endif

  @if (isset($acknowledgmentMap[$row->id]))
  <i class="fas fa-file-contract text-success" data-bs-toggle="tooltip" title="{{ $acknowledgmentMap[$row->id] }}"></i>
  @endif
</div>
