        ...
        <div class="col-12 col-md-6 col-lg-4">
          <div class="card h-100 shadow-sm hover-shadow transition">
            ...
            <div class="position-relative bg-light" style="padding-top: 75%; overflow: hidden;">
              <img src="{{ $image->image_url }}" alt="Cover Image" class="position-absolute top-0 start-0 w-100 h-100" style="object-fit: contain;" onerror="this.onerror=null; this.src='/images/placeholder.png';">
              ...
              <div class="position-absolute bottom-0 end-0 m-2 d-flex align-items-center gap-2">
                {{-- Status Toggle --}}
                <div class="form-check form-switch mb-0 bg-white rounded px-2 py-1" style="box-shadow: 0 2px 8px rgba(0,0,0,0.15);">
                  <input class="form-check-input" type="checkbox" role="switch" id="status-{{ $image->id }}" {{ $image->status ? 'checked' : '' }} onchange="updateStatus({{ $image->id }})" title="{{ __('messages.common.toggle_status') ?? 'Toggle Status' }}">
                </div>

                {{-- Action Buttons --}}
                <div class="btn-group btn-group-sm" role="group" style="box-shadow: 0 2px 8px rgba(0,0,0,0.15);">
                  <button type="button" class="btn btn-light" onclick="deleteCoverImage({{ $image->id }})" title="{{ __('messages.common.delete') }}">
                    <i class="fa fa-trash text-danger"></i>
                  </button>
                  <button type="button" class="btn btn-light" onclick="editCoverImage({{ $image->id }})" title="{{ __('messages.common.edit') }}">
...
<div class="modal fade" id="addCoverImageModal" tabindex="-1" aria-labelledby="addCoverImageModalLabel" aria-hidden="true">
      ...
      <form action="{{ route('cover-images.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          <div class="mb-3">
            ...
            <div class="form-text">
              <i class="fa fa-info-circle me-1"></i>{{ __('messages.cover_image.allowed_types') ?? 'Allowed: JPG, PNG, JPEG (Max: 5MB)' }}
            </div>
            {{-- Image Paste Component --}}
            <div data-image-paste data-file-input-id="image" data-preview-id="addImagePreview" data-button-text="{{ __('messages.select_image') }}" data-clipboard-button-text="{{ __('messages.paste_from_clipboard') }}" data-success-text="{{ __('messages.image_pasted_successfully') }}" data-invalid-type-text="{{ __('messages.invalid_image_type') }}" data-image-too-large-text="{{ __('messages.image_too_large') }}" data-no-image-text="{{ __('messages.no_image_in_clipboard') }}">
            </div>
            {{-- Image Preview --}}
            <div id="addImagePreviewContainer" class="mt-3" style="display:none;">
              <div id="addImagePreview" style="width: 100%; max-width: 600px; height: 200px; background-size: contain; background-position: center; background-repeat: no-repeat; border: 2px solid #ddd; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
              </div>
            </div>
          </div>
        </div>
...
<div class="modal fade" id="editCoverImageModal" tabindex="-1" aria-labelledby="editCoverImageModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <form id="editCoverImageForm" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          <div class="mb-3">
            ...
            {{-- Image Paste Component --}}
            <div data-image-paste data-file-input-id="edit_image" data-preview-id="editImagePreview" data-button-text="{{ __('messages.select_image') }}" data-clipboard-button-text="{{ __('messages.paste_from_clipboard') }}" data-success-text="{{ __('messages.image_pasted_successfully') }}" data-invalid-type-text="{{ __('messages.invalid_image_type') }}" data-image-too-large-text="{{ __('messages.image_too_large') }}" data-no-image-text="{{ __('messages.no_image_in_clipboard') }}">
            </div>
            {{-- Image Preview --}}
            <div id="editImagePreviewContainer" class="mt-3" style="display:none;">
              <div id="editImagePreview" style="width: 100%; max-width: 600px; height: 200px; background-size: contain; background-position: center; background-repeat: no-repeat; border: 2px solid #ddd; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  function deleteCoverImage(id) {
    if (confirm(
        '{{ __('
        messages.cover_image.confirm_delete ') ?? '
      ...
      .then(data => {
        if (data.success) {
          ...
          if (badge) {
            ...
            badge.textContent = data.status ? '{{ __('
